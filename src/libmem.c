...
int __free(struct pcb_t *caller, int vmaid, int rgid)
{
  ...
  if (!rgnode || rgnode->rg_start >= rgnode->rg_end) return -1;
  ...
  printf("Free region for vmaid %d", vmaid);
...
int pg_getpage(struct mm_struct *mm, int pgn, int *fpn, struct pcb_t *caller)
{
  ...
  { /* Page is not online, make it actively living */
    ...
    regs_out.a2 = vicfpn;   // from RAM
    regs_out.a3 = swpfpn;   // to SWAP
   ...
   */
    if (PAGING_PAGE_SWAPPED(tgtpte)) {
      ...
      regs_in.a2 = tgt_swpfpn;  // Source in SWAP
      regs_in.a3 = vicfpn;      // Destination in RAM
...
int pg_getval(struct mm_struct *mm, int addr, BYTE *data, struct pcb_t *caller)
{
   ...
   */
  int phyaddr = (fpn << PAGING_ADDR_FPN_LOBIT) + off; // Calculate physical address: FPN * page_size + offset
  struct sc_regs regs;
...
{
  struct vm_area_struct *cur_vma = get_vma_by_num(caller->mm, vmaid);
  if (!cur_vma || !cur_vma->vm_freerg_list) return -1;
    ...
    if (region_size >= size) {
        ...
        if (region_size > size) {
            ...
            if (prev == NULL) {
                ...
                prev->rg_next = rgit->rg_next; // Bypass current node
            }
            free(rgit); // Free the node as it's no longer needed
        }
